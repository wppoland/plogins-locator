...
/**
 * Plugin Name:       Lokilo - Store Locator for WooCommerce
    ...
    add_action('admin_notices', static function (): void {
        printf(
            ...
            esc_html(sprintf(
                ...
                __('Lokilo requires PHP %1$s or higher. You are running PHP %2$s.', 'lokilo'),
                MIN_PHP_VERSION,
                PHP_VERSION,
            )),
        );
...
add_action('plugins_loaded', static function (): void {
    if (! class_exists('WooCommerce')) {
        add_action('admin_notices', static function (): void {
            printf(
                '<div class="notice notice-error"><p>%s</p></div>',
                esc_html__('Lokilo requires WooCommerce to be installed and activated.', 'lokilo'),
            );
        });
        return;
    }
